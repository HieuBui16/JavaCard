...
public class ECDHApplet extends Applet {
	...
	protected ECDHApplet() {
		...
		p1PrivateKey = (ECPrivateKey) KeyBuilder.buildKey(KeyBuilder.TYPE_EC_FP_PRIVATE, SECP256k1.SECP256K1_KEY_SIZE, false);
		p1PublicKey = (ECPublicKey) KeyBuilder.buildKey(KeyBuilder.TYPE_EC_FP_PUBLIC, SECP256k1.SECP256K1_KEY_SIZE, false);
		p2PrivateKey = (ECPrivateKey) KeyBuilder.buildKey(KeyBuilder.TYPE_EC_FP_PRIVATE, SECP256k1.SECP256K1_KEY_SIZE, false);
		p2PublicKey = (ECPublicKey) KeyBuilder.buildKey(KeyBuilder.TYPE_EC_FP_PUBLIC, SECP256k1.SECP256K1_KEY_SIZE, false);
		p1ShareKey = (AESKey) KeyBuilder.buildKey(KeyBuilder.TYPE_AES_TRANSIENT_DESELECT, KeyBuilder.LENGTH_AES_128, false);
		...
		p1PrivateKey.setS(new byte[] {(byte) 0xe9, (byte) 0x6c, (byte) 0x2b, (byte) 0x06, (byte) 0xfc, (byte) 0x82, (byte) 0xd8, (byte) 0x62, (byte) 0x43, (byte) 0x2f,
	...
	public void process(APDU apdu) {
		...
		secureChannel.receive(buffer, ISO7816.OFFSET_CDATA, size, buffer, ISO7816.OFFSET_CDATA);
	...
	/* generate Secret key from priKey of party 1 and pubKey of party 2
	 * use a random byte called 'seed' to increase the secure appended in the original secret key derived from the key pair
	 * then hash the result using sha256 algorithm to generate the 256 bits (32 bytes) key which is the final secret key
	 */
...
//		apdu.sendBytesLong(temSecret, (short) 0, (short) (short) temSecret.length);
	 ...
	 * then encrypt the result using AES_CBC algorithm 
	 * the input is the half of the hashed message cz the block size required 128 bits (16 bytes)
	 */
	private void p1EncAndSendP2(APDU apdu) throws CryptoException {
		...
		short mgLen = cipher.doFinal(buffer, (short) (ISO7816.OFFSET_CDATA + 16), (short) (mg - 16), temmg, (short) mgLen0);
	...
	private void inSecret(APDU apdu) {
		byte[] buffer = apdu.getBuffer();
		short size = apdu.setIncomingAndReceive();
		p1ShareKey.setKey(buffer, ISO7816.OFFSET_CDATA);
